...
// ─── MODAL ──────────────────────────────────────────────────────
interface ModalData { name: string; ingredients?: string; }
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-box" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-body">
          ...
          {data.ingredients && (
            <div className="modal-ingredients">{data.ingredients}</div>
          )}
        </div>
      </div>
    </div>
...
// ─── SIGNATURE CARD ──────────────────────────────────────────────
function SigCard({ item, onInfo }: { item: SignatureCocktail; onInfo?: () => void }) {
  return (
    <div className="sig-card">
      <div className="sig-body">
        <div className="sig-name">{item.name}</div>
        {item.ingredients && <div className="sig-ingredients">{item.ingredients}</div>}
      </div>
...
const CATS: { id: MainCat; icon: string; labelKey: keyof Translation }[] = [
      ...
      if (currentSub === 'signature') return (
        <>
          ...
          {signatureCocktails.map((i) => (
            <SigCard key={i.name} item={i} onInfo={() => openModal({ name: i.name, ingredients: i.ingredients })} />
          ))}
    ...
    <>
      ...
      <div className="footer">
        ...
        <div className="social-btns">
          ...
          <a href="https://wa.me/393280000000" target="_blank" rel="noopener noreferrer" className="social-btn social-whatsapp">
          ...
          <a href="https://g.page/r/zoepremiumbar/review" target="_blank" rel="noopener noreferrer" className="social-btn social-review">
